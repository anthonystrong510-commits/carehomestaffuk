...
/**
 * Card showing the business WhatsApp number with copy-to-clipboard and a
 * "open blank chat" action. Opens a bare wa.me link (no pre-filled text) so
 * the applicant starts the conversation themselves — avoids WhatsApp spam flags.
 */
export function WhatsAppContactCard({ number, className = "" }: { number: string; className?: string }) {
  ...
  const copy = async () => {
    try {
      ...
      // Clipboard API blocked — fall back to a hidden textarea copy
    ...
    setCopied(true);
    toast({ title: "Number copied", description: pretty });
    setTimeout(() => setCopied(false), 2000);
  };

  const openBlank = () => {
    // Opens the gate first (UK number verification) if needed; then a bare wa.me link.
    openWhatsAppGated(num);
  };

  return (
    <div className={`rounded-xl border bg-card p-4 sm:p-5 space-y-3 ${className}`}>
      <div className="flex items-center gap-3">
        <span className="inline-flex h-10 w-10 items-center justify-center rounded-full bg-[#25D366] text-white shrink-0">
          <svg viewBox="0 0 32 32" className="h-5 w-5" fill="currentColor" aria-hidden="true">
            <path d="M19.11 17.34c-.29-.14-1.71-.84-1.97-.94-.27-.1-.46-.14-.66.14-.19.28-.75.94-.93 1.13-.17.19-.34.21-.63.07-.29-.14-1.21-.45-2.31-1.42-.85-.76-1.43-1.7-1.6-1.99-.17-.29-.02-.45.13-.59.13-.13.29-.34.43-.51.14-.17.19-.29.29-.48.1-.19.05-.36-.02-.51-.07-.14-.66-1.59-.9-2.18-.24-.57-.49-.49-.66-.5-.17-.01-.36-.01-.55-.01-.19 0-.5.07-.77.36-.27.28-1.02 1-1.02 2.43s1.05 2.83 1.2 3.02c.14.19 2.06 3.15 5 4.41.7.3 1.24.48 1.67.61.7.22 1.34.19 1.85.12.56-.08 1.71-.7 1.95-1.38.24-.68.24-1.27.17-1.39-.07-.12-.26-.19-.55-.33zM16.02 5.33c-5.9 0-10.7 4.79-10.7 10.69 0 1.89.49 3.74 1.43 5.36L5 26.67l5.41-1.42c1.56.85 3.32 1.3 5.11 1.3h.01c5.9 0 10.7-4.79 10.7-10.69 0-2.85-1.11-5.54-3.13-7.55-2.02-2.02-4.7-3.13-7.56-3.13z" />
          </svg>
        </span>
        <div className="min-w-0">
          <p className="text-sm font-semibold text-foreground">Message our recruitment team on WhatsApp</p>
          <p className="text-lg font-bold tracking-wide text-foreground tabular-nums">{pretty}</p>
        </div>
      </div>
      <p className="text-xs text-muted-foreground">
        Copy the number and start the chat yourself — we never send automated first messages. Only contact this exact number; anyone else claiming to be us is not authorised.
      </p>
      <div className="flex flex-wrap gap-2">
        <Button type="button" variant="outline" size="sm" onClick={copy}>
          {copied ? "Copied!" : "Copy number"}
        </Button>
        <Button type="button" size="sm" className="bg-[#25D366] hover:bg-[#1ebe5a] text-white" onClick={openBlank}>
          Open blank chat
        </Button>
      </div>
    </div>
